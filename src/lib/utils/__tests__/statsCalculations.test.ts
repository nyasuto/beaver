...
describe('StatsCalculations.extractPriorityFromLabels', () => {
  it('should extract critical priority correctly', () => {
    ...
    criticalLabels.forEach(labels => {
      const priority = StatsCalculations.extractPriorityFromLabels(labels);
      expect(priority).toBe('critical');
    });
  });

  it('should extract high priority correctly', () => {
    const highLabels = [
      [{ name: 'priority:high' }],
      [{ name: 'High Priority' }],
      [{ name: 'important' }],
    ...
    highLabels.forEach(labels => {
      const priority = StatsCalculations.extractPriorityFromLabels(labels);
      expect(priority).toBe('high');
    });
  });

  it('should extract medium priority correctly', () => {
    const mediumLabels = [
  ...
  it('should default to low priority', () => {
    ...
    lowPriorityLabels.forEach(labels => {
      const priority = StatsCalculations.extractPriorityFromLabels(labels);
  ...
  it('should prioritize higher priority when multiple priorities exist', () => {
    const labels = [
      { name: 'priority:low' },
      { name: 'priority:critical' },
      { name: 'priority:medium' },
