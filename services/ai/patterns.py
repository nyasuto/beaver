...
class PatternRecognitionEngine:
    ...
    def __init__(self):
        ...
        self.success_keywords = [
            'fix', 'solve', 'implement', 'complete', 'success', 'work',
            'merge', 'deploy', 'release', 'stable', 'ready'
        ]
        
        self.failure_keywords = [
            'error', 'bug', 'fail', 'issue', 'problem', 'broken',
            'revert', 'rollback', 'crash', 'wrong', 'mistake'
        ]
        
        self.learning_keywords = [
            'learn', 'try', 'experiment', 'test', 'explore', 'research',
            'investigate', 'study', 'understand', 'figure out', 'discover'
        ]
        
        self.improvement_keywords = [
            'improve', 'optimize', 'refactor', 'enhance', 'upgrade',
            'better', 'faster', 'efficient', 'clean', 'update'
        ]
    ...
    def _detect_success_patterns(self, events: List[DevelopmentEvent]) -> List[LearningPattern]:
        ...
        if quick_resolutions:
            pattern = LearningPattern(
                ...
                evidence=[f"Issue #{event.metadata.get('issue_number', event.id)} resolved quickly" for event in quick_resolutions[:5]],
    ...
    def _detect_failure_recovery_patterns(self, events: List[DevelopmentEvent]) -> List[LearningPattern]:
        """失敗からの回復パターンの検出"""
        patterns = []
    ...
    def _find_failure_recovery_sequences(self, events: List[DevelopmentEvent]) -> List[Dict[str, Any]]:
        """失敗からの回復シーケンスを検出"""
        recoveries = []
        ...
        for i, event in enumerate(events[:-2]):
            ...
            if any(keyword in text for keyword in self.failure_keywords):
                ...
                if learning_events and success_events:
                    recovery = {
                        'id': f"recovery_{i}",
                        'description': f"失敗後の学習プロセスを経て成功に至ったパターン",
                        'confidence': 0.8,
                        'evidence': [
                        ...
                        'success_rate': len(success_events) / (len(learning_events) + len(success_events))
                    }
    ...
    def _extract_tech_domains(self, events: List[DevelopmentEvent]) -> Dict[str, List[DevelopmentEvent]]:
        """技術ドメインを抽出"""
        tech_domains = {}
    ...
    def _analyze_skill_evolution(self, domain: str, events: List[DevelopmentEvent]) -> Optional[Dict[str, Any]]:
        """特定ドメインでのスキル進化を分析"""
        if len(events) < 5:
            return None
...
def create_development_events_from_issues(issues_data: List[Dict[str, Any]]) -> List[DevelopmentEvent]:
