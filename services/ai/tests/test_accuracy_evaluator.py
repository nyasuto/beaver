...
class TestAccuracyEvaluator:
    ...
    def test_benchmark_test_cases_diversity(self, accuracy_evaluator):
        ...
        japanese_issues = [
            ...
            if any(
                ...
                or "\u30a0" <= char <= "\u30ff"
                or "\u4e00" <= char <= "\u9faf"
                for char in issue.title + issue.body
            )
        ]
    ...
    async def test_calculate_detailed_metrics_with_errors(
        ...
        assert (
            metrics.precision_per_category["feature-request"] == 0.0
        )  # 0 TP, 0 FP (no predictions)
...
class TestBenchmarkTestCases:
    ...
    def test_benchmark_edge_cases(self, accuracy_evaluator):
        ...
        for case in test_cases:
            ...
            if (
                "refactor" in title_lower or "improve" in title_lower
            ) and expected_category != "architecture":
                ambiguous_cases.append(case)
            elif (
                "research" in title_lower or "investigate" in title_lower
            ) and expected_category != "learning":
                ambiguous_cases.append(case)
