...
class TestEnhancedClassificationService:
    ...
    @pytest.mark.asyncio
    async def test_classify_issue_enhanced_success(self, enhanced_classification_service, sample_issue):
    ...
    @pytest.mark.asyncio
    async def test_classify_issue_enhanced_with_few_shot(self, enhanced_classification_service, sample_issue):
    ...
    async def test_batch_classify_issues_enhanced(self, enhanced_classification_service):
        ...
        issues = [
            ...
            Issue(id=2, title="Feature request", body="Add new feature", labels=[], repository="test")
        ]
        
        mock_results = [
            ClassificationResult(
                issue_id=1, category="bug-fix", confidence=0.9, 
                reasoning="Bug", suggested_tags=[], processing_time_ms=200
            ),
            ClassificationResult(
                issue_id=2, category="feature-request", confidence=0.8,
                reasoning="Feature", suggested_tags=[], processing_time_ms=180
            )
        ]
        
        with patch.object(enhanced_classification_service, 'classify_issue_enhanced') as mock_classify:
    ...
    async def test_evaluate_classification_accuracy(self, enhanced_classification_service):
        ...
        with patch.object(enhanced_classification_service, 'batch_classify_issues_enhanced') as mock_batch:
