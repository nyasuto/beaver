    ...
    def __init__(self):
        ...
        self.enhanced_categories = {
            "bug-fix": {
                ...
                "keywords_ja": ["バグ", "エラー", "不具合", "異常", "例外", "修正", "直す", "動かない", "失敗"],
                "keywords_en": ["bug", "error", "crash", "fix", "broken", "fails", "exception", "defect", "malfunction"],
                "indicators": ["stack trace", "error message", "reproducible steps", "expected vs actual"]
            },
            "feature-request": {
                "name_ja": "機能要求",
                ...
                "keywords_ja": ["機能", "追加", "新しい", "改善", "拡張", "要求", "提案", "実装", "できるように"],
                "keywords_en": ["feature", "enhancement", "add", "improve", "request", "new", "implement", "ability"],
            ...
            "architecture": {
                ...
                "keywords_ja": ["設計", "アーキテクチャ", "リファクタリング", "構造", "性能", "最適化", "テスト", "技術的負債"],
                "keywords_en": ["design", "architecture", "refactor", "structure", "performance", "optimization", "test", "debt"],
                "indicators": ["design pattern", "code structure", "performance metrics", "test coverage"]
            },
            "learning": {
                "name_ja": "学習・調査",
                ...
                "keywords_ja": ["調査", "研究", "学習", "実験", "プロトタイプ", "検証", "試す", "勉強"],
                "keywords_en": ["research", "investigate", "study", "learn", "experiment", "prototype", "explore", "poc"],
                "indicators": ["proof of concept", "experiment", "research", "comparison", "evaluation"]
            },
            "troubleshooting": {
                "name_ja": "トラブルシューティング",
                ...
                "keywords_ja": ["質問", "ヘルプ", "サポート", "使い方", "設定", "環境", "インストール", "わからない"],
                "keywords_en": ["help", "support", "question", "how to", "setup", "configuration", "installation", "documentation"],
                "indicators": ["how to", "configuration", "environment", "documentation", "tutorial"]
            }
        }
...
```"""
        ...
        if use_few_shot and self.few_shot_examples:
            relevant_examples = [ex for ex in self.few_shot_examples 
                               if ex.language == language or ex.language == Language.AUTO][:3]
...
```"""
                ...
                messages.extend([
                    ...
                    SystemMessage(content=example_output)
                ])
...
Repository: {issue.repository or 'N/A'}"""
    ...
    def evaluate_classification_accuracy(self, test_cases: List[Tuple[Issue, str]]) -> ClassificationMetrics:
        """分類精度を評価"""
        # 実装は分類サービスと連携して行う
        # ここでは評価フレームワークの構造を定義
        ...
        # TODO: 実際の評価ロジックを実装
        logger.info(f"Classification accuracy evaluation completed for {len(test_cases)} test cases")
