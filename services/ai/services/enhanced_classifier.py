...
class EnhancedClassificationService:
    ...
    async def classify_issue_enhanced(
        self, issue: Issue, config: Optional[ClassificationConfig] = None, use_few_shot: bool = True
    ...
    def get_performance_summary(self) -> dict[str, Any]:
        ...
        return {
            ...
            "target_response_time_met": sum(1 for t in response_times if t <= 3000)
            / len(response_times),
    ...
    async def evaluate_classification_accuracy(
        ...
        logger.info(
            f"Starting classification accuracy evaluation with {len(test_issues)} test cases"
        )
        ...
        average_f1_score = (
            sum(f1_score_per_category.values()) / len(f1_score_per_category)
            if f1_score_per_category
            else 0.0
        )
