...
class AIClient:
    ...
    def _prepare_issue_content(self, issue: IssueData, include_comments: bool = True, max_length: int = 15000) -> str:
...
Specific next steps or solutions"""
        ...
        for attempt in range(3):  # Retry up to 3 times
            try:
                response = await client.chat.completions.create(
                    model=model,
                    messages=[
                        {"role": "system", "content": "You are an expert technical analyst specializing in GitHub issue analysis and summarization."},
                ...
                logger.warning(f"OpenAI rate limit hit (attempt {attempt + 1}/3): {error_info.message}")
                ...
                if e.status_code >= 500:  # Server errors - retry
                    logger.warning(f"OpenAI server error (attempt {attempt + 1}/3): {e.status_code} - {str(e)}")
                    if attempt < 2:
                ...
                if attempt < 2:
                    ...
                    raise ValueError(f"OpenAI request timeout after 3 attempts")
        ...
        for attempt in range(3):  # Retry up to 3 times
            try:
                response = await client.messages.create(
                    ...
                    temperature=temperature,
                    messages=[
                        {"role": "user", "content": f"You are an expert technical analyst. {prompt}"}
                    ]
                )
                ...
                logger.warning(f"Anthropic rate limit hit (attempt {attempt + 1}/3): {error_info.message}")
                ...
                if e.status_code >= 500:  # Server errors - retry
                    logger.warning(f"Anthropic server error (attempt {attempt + 1}/3): {e.status_code} - {str(e)}")
                    if attempt < 2:
                        await asyncio.sleep(2 ** attempt)
                        continue
                else:  # Client errors - don't retry
                ...
                if attempt < 2:
                    ...
                    raise ValueError(f"Anthropic request timeout after 3 attempts")
        ...
        try:
            ...
            for line in lines:
                ...
                if line.startswith('## 要約') or line.startswith('## Summary'):
                    ...
                    continue
                elif line.startswith('## 推奨アクション') or line.startswith('## Recommended Actions'):
    ...
    def _parse_openai_rate_limit_error(self, error: openai.RateLimitError) -> APIError:
        ...
        quota_exceeded = any(phrase in error_message.lower() for phrase in [
            "quota exceeded", "insufficient quota", "billing", "usage limit"
        ])
