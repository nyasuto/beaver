...
class Settings(BaseSettings):
    ...
    max_content_length: int = Field(
        default=50000, description="Maximum content length for processing"
    )
    ...
    enable_troubleshooting: bool = Field(
    ...
    def _validate_ai_keys(self):
        ...
        if not self.openai_api_key and not self.anthropic_api_key:
            ...
            if not openai_key and not anthropic_key:
                raise ValueError(
                    "At least one AI API key must be provided: OPENAI_API_KEY or ANTHROPIC_API_KEY"
                )
