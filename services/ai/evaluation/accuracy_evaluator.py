...
class AccuracyEvaluator:
    ...
    async def _calculate_detailed_metrics(
        ...
        # 成功した分類のみを対象
        valid_results = [(exp, res) for exp, res in zip(expected_categories, results) if res is not None]
    ...
    async def _generate_evaluation_report(
        ...
        # CSVレポートも作成
        await self._generate_csv_report(report, report_path / f"classification_evaluation_{timestamp_str}.csv")
    ...
    async def _generate_csv_report(self, report: Dict[str, Any], csv_path: Path) -> None:
        ...
        with open(csv_path, 'w', newline='', encoding='utf-8') as csvfile:
            fieldnames = [
                'test_case_id', 'issue_id', 'issue_title', 'expected_category',
                'predicted_category', 'confidence', 'correct', 'processing_time_ms', 'reasoning'
            ]
    ...
    def get_evaluation_trends(self) -> Dict[str, Any]:
        ...
        trends = {
            "accuracy": {
                ...
                "trend": "improving" if accuracy_trend > 0 else "declining" if accuracy_trend < 0 else "stable"
            },
            "f1_score": {
            ...
            "response_time": {
                ...
                "trend": "improving" if response_time_trend < 0 else "declining" if response_time_trend > 0 else "stable"
            },
...
def create_accuracy_evaluator(classification_service: EnhancedClassificationService) -> AccuracyEvaluator:
    """精度評価システムのファクトリ関数"""
