...
class TestGitHubAPIIntegration:
    ...
    def test_repository_access(self, github_token, test_repository):
        ...
        assert verification["can_read_issues"], \
            f"Cannot read issues from {test_repository}"
        assert verification["rate_limit_ok"], \
            "Insufficient rate limit for testing"
        
        # Repository should have some basic properties
        assert isinstance(verification["issues_count"], int), \
            "Issue count should be a number"
        assert isinstance(verification["languages"], dict), \
            "Languages should be a dictionary"
    ...
    def test_rate_limit_handling(self, github_token, test_repository):
        ...
        initial_remaining = initial_rate_limit.get("resources", {}).get("core", {}).get("remaining", 0)
...
class TestBeaverGitHubIntegration:
    ...
    @pytest.mark.github_api
    @pytest.mark.slow
    def test_beaver_issue_processing(self, beaver_binary, beaver_config, test_repository, github_token):
    ...
    @pytest.mark.github_api
    @pytest.mark.slow
    def test_beaver_end_to_end_workflow(self, beaver_binary, beaver_config, test_repository, github_token, cleanup_test_files):
...
"""
        ...
        assert "unauthorized" in result.stderr.lower() or "token" in result.stderr.lower() or \
               "認証" in result.stderr or "トークン" in result.stderr, \
    ...
    def test_nonexistent_repository(self, beaver_binary, beaver_config, github_token):
        ...
        assert "not found" in stderr.lower() or "404" in stderr or \
               "見つかりません" in stderr or "存在しません" in stderr, \
...
"""
        ...
        assert "panic" not in result.stderr.lower(), f"Permission issue caused panic: {result.stderr}"
        
        # Either succeeds or fails gracefully
        if result.returncode != 0:
            assert "forbidden" in result.stderr.lower() or "permission" in result.stderr.lower() or \
                   "access" in result.stderr.lower() or "権限" in result.stderr, \
