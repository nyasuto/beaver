...
class TestSiteGeneration:
    ...
    def test_site_build_basic(self, beaver_binary, beaver_config, github_token, cleanup_test_files):
        ...
        with tempfile.TemporaryDirectory(prefix="beaver_site_test_") as temp_dir:
            ...
            if success:
                ...
                assert len(structure["errors"]) == 0, f"Site structure errors: {structure['errors']}"
    
    @pytest.mark.slow
    def test_site_content_verification(self, beaver_binary, beaver_config, github_token, cleanup_test_files):
    ...
    @pytest.mark.slow
    def test_beaver_specific_content(self, beaver_binary, beaver_config, github_token, cleanup_test_files):
    ...
    @pytest.mark.slow
    def test_github_pages_compatibility(self, beaver_binary, beaver_config, github_token, cleanup_test_files):
        ...
        with tempfile.TemporaryDirectory(prefix="beaver_pages_test_") as temp_dir:
            ...
            assert compat["has_index_html"], \
                "Site should have index.html for GitHub Pages"
    ...
    def test_site_css_assets(self, beaver_binary, beaver_config, github_token, cleanup_test_files):
        ...
        with tempfile.TemporaryDirectory(prefix="beaver_css_test_") as temp_dir:
            ...
            if css_verification["css_files_found"]:
                assert css_verification["total_css_size"] > 0, \
                    "CSS files should have content"
...
class TestSiteGenerationErrorHandling:
    ...
    def test_invalid_output_directory(self, beaver_binary, beaver_config):
        ...
        assert "permission" in stderr.lower() or "path" in stderr.lower() or \
               "directory" in stderr.lower() or "ディレクトリ" in stderr, \
...
class TestSiteGenerationPerformance:
    ...
    @pytest.mark.slow
    def test_large_content_handling(self, beaver_binary, beaver_config, github_token, cleanup_test_files):
        ...
        with tempfile.TemporaryDirectory(prefix="beaver_large_test_") as temp_dir:
            ...
            if success:
                ...
                assert structure["site_directory_exists"], "Large content should still generate site"
    
    @pytest.mark.slow
    def test_incremental_build_performance(self, beaver_binary, beaver_config, github_token, cleanup_test_files):
