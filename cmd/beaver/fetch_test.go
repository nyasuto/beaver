				...
				if tt.outputFile != "" {
					...
					for _, expectedOutput := range tt.expectOutput {
						if !strings.Contains(expectedOutput, "✅") {
							...
							assert.Contains(t, string(content), strings.Replace(expectedOutput, "\"", "", -1), "File should contain expected JSON content")
						}
					}
				} else {
					// Check stdout output
...
func TestRepositoryValidation(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			// Test with actual parseOwnerRepo function
			owner, repo := parseOwnerRepo(tt.repository)
			if tt.expectError {
				assert.Empty(t, owner, "Owner should be empty for invalid repository")
				assert.Empty(t, repo, "Repo should be empty for invalid repository")
			} else {
				assert.NotEmpty(t, owner, "Owner should not be empty for valid repository")
				assert.NotEmpty(t, repo, "Repo should not be empty for valid repository")

				// Verify the split is correct
				parts := strings.Split(tt.repository, "/")
				if len(parts) == 2 {
					assert.Equal(t, parts[0], owner, "Owner should match first part")
					assert.Equal(t, parts[1], repo, "Repo should match second part")
				}
			}
		})
	}
